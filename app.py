...
def show_document(html_content):
    """
    Renders the document inside an iframe in the page.
    The iframe has its own print button that calls window.print()
    scoped to the iframe — works without any popup blocker issues.
    """
    b64 = base64.b64encode(html_content.encode("utf-8")).decode()
    st.markdown("---")
    st.markdown("#### 📄 Documento generado — usa el botón rojo para imprimir / guardar PDF")

    # Embed document in iframe — no new tab needed, no popup blocked
    iframe_html = f"""
    <iframe
        id="docframe"
    ...
    <p style="font-size:11px;color:#888;margin-top:6px">
      💡 Haz clic en el botón rojo dentro del documento &rarr; <b>Ctrl+P</b> &rarr;
      Destino: <b>Guardar como PDF</b> &rarr; Sin m&aacute;rgenes &rarr; Guardar
    </p>
    """
...
st.markdown("""
<style>
.block-container { padding-top: 1.8rem; max-width: 760px; }
...
if st.button("📄 Generar Documento", type="primary", use_container_width=True):
    required = {"Date": fecha, "MAWB": mawb_full_input,
